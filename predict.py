...
# Set paths
valid_english_path = args["language_src"]
valid_vietnamese_path = args["language_targ"]
...
with open(valid_english_path,'r', encoding='utf-8') as f:
    inputSeq = f.read().split('\n')

#inputSeq = []
#inputSeq.append('what are you doing?')
#inputSeq.append('what is your name?')
#inputSeq = inputSeq
...
with tf.Session() as sess:  
    ...
    loaded_model=Seq2SeqModel(len(int2word_english), len(int2word_vietnamese), word_embed_english, word_embed_vietnamese , english_max_len, vietnamese_max_len, params, train=False)
    ...
    real_len = list(map(lambda seq: len([word_int for word_int in seq if word_int != 0]), input_data))
